...
Suppose the alphabet contains n symbols, and the i-th symbol occurs p[i] times.

1. Initially, each symbol is considered an **active** node of the future Huffman tree, labeled with its frequency p[i].
...
sum for i=1 to n of (p[i] * code_length[i])
...
- The first line contains an integer n, the number of symbols  
  2 ≤ n ≤ 2,500,000
- The second line contains n integers p[i] — the frequencies of the symbols  
  1 ≤ p[i] ≤ 10^9  
  The sequence is sorted non-decreasingly: p[i] ≤ p[i+1]
