...
A **binomial tree** of degree k, denoted Bk, is defined recursively:

- B0 is a single vertex.
- For k ≥ 1, Bk is formed by linking two copies of B(k-1): make the root of one copy a child of the root of the other.

Equivalently, the root of Bk has children that are the roots of binomial trees B0, B1, ..., B(k-1).  
The number of nodes in Bk is 2^k.

A **binomial heap** is a collection of binomial trees of pairwise distinct degrees. That is, for each k ≥ 0, either Bk appears in the heap exactly once or it does not appear at all.
