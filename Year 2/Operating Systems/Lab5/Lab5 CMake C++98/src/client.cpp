...
int main()
{
    ...
    while (true)
    {
        ...
        if (choice == 'r' || choice == 'm')
        {
            ...
            // Открываем соединение для каждой операции
            ...
            employee emp;
            ZeroMemory(&emp, sizeof(emp)); // очистка перед чтением
            DWORD bytesRead;
            ...
            if (choice == 'r')
            {
                // For read: after user views, send 'f' to finish reading
            ...
            else // modify
            {
                ...
                for (size_t i = len; i < 10; ++i) emp.name[i] = '\0';
                ...
                // after sending modified record, send finish
                char finish = 'f';
                if (!WriteFile(hPipe, &finish, sizeof(finish), &bw, NULL) || bw != sizeof(finish))
                {
                    std::cerr << "WriteFile(finish) failed. Error: " << GetLastError() << "\n";
